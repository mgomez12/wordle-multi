...
const Game = (props) => {
  ...
  const inputChar = async (e) => {
    ...
    if (
      (String.fromCharCode(e.keyCode).match(/[a-zA-Z]/g) &&
        curGuess.length < 5) ||
      e.key == "Backspace"
    )
      updateGuess(guesses.length, e.key);
    else if (e.key == "Enter") {
      console.log("GUESSING");
      if (curGuess.length < 5) return;
      let word_colors = (
        await post("/api/guess", { userid: props.userId, word: curGuess })
      ).colors;
  ...
  useEffect(() => {
    ...
    if (props.gameId && props.userId) {
      ...
      get("/api/validGame", { id: props.gameId, user: props.userId }).then(
        (resp) => {
  ...
  const showResults = () => {
    ...
    for (const p in results.points)
      points.push({ player: p, points: results.points[p] });
    points.sort((a, b) => (b.points > a.points ? 1 : -1));
    return points.map((point) => (
      <div key={point.player}>
        {" "}
        <b> {gameState.names[point.player]}: </b> {point.points}{" "}
      </div>
    ));
  };
  ...
  const renderGame = (e) => {
    return (
      <>
        ...
        <div style={{ textAlign: "center" }}>Time Left: {timeFormat(time)}</div>
        {invalidWord ? (
          <Alert variant="danger">Invalid word!!! </Alert>
        ) : (
          <div />
        )}
          ...
          {!joined ? (
            <Modal.Body>
              <InputGroup>
                ...
                <Button disabled={playerName == ""} onClick={joinGame}>
                  {" "}
                  Join{" "}
                </Button>
              </InputGroup>
    ...
    <>
      {!validSession ? (
        ...
        >
          ...
          {warning ? <Alert variant="danger"> {warningText} </Alert> : <div />}
        </div>
      ) : (
        renderGame()
      )}
