...
var fs = require("fs");
var solutions = fs.readFileSync(
  require("path").resolve(__dirname, "solutions.txt")
);
var valid_words = fs.readFileSync(
  require("path").resolve(__dirname, "valid-wordle-words.txt")
);
...
const createGame = () => {
  ...
  do {
    gameid = "";
    for (let i = 0; i < 6; i++)
      gameid += randLetter = String.fromCharCode(
...
const startGame = (gameid) => {
  ...
  state.word =
    solutions[Math.trunc(Math.random() * solutions.length)].toUpperCase();
  state.guesses = {};
...
const guess = (gameid, userid, guess) => {
  ...
  if (colors == "YYYYY") {
    ...
    points +=
      40 - 10 * Math.floor((Date.now() - state.timeStarted) / (30 * 1000));
